...
## CORRECTED (2026-08-22): RoboTwin training data covers all 50/50 canonical tasks, roughly uniformly -- the earlier "46/50, 4 tasks with zero data" claim below was wrong

**This section previously claimed 4 canonical tasks (`handover_block`, `handover_mic`, `blocks_ranking_rgb`, `blocks_ranking_size`) had zero training episodes, based on a naive single-keyword substring search (e.g. literally searching for the token "handover"). That claim was independently re-verified on a fresh machine and found to be false.**

Using RoboTwin's own official per-task instruction templates (`third_party/RoboTwin/description/task_instruction/*.json` -- the actual generation source for the paraphrased training instructions, not guessed keywords) as ground truth, and matching multi-word template fragments (not single generic words) against all 27,500 episodes in `meta/episodes.jsonl`: **all 50 canonical tasks have real matched episodes**, including all 4 previously-flagged tasks (591-1100 matched episodes each, e.g. `handover_block` matched real instructions like "Take the red block using the left arm and hand it over."). `27,500 total episodes / 50 tasks = 550 exactly` -- the dataset is a clean uniform 550-episodes-per-task split by design, not a partial-coverage set. Every task's floor is >=550 matched episodes; counts above that reflect template-fragment over-matching in the verification script (generic shared phrases across tasks), not real extra coverage, and should not be trusted as precise ranked counts -- but the floor finding (no task at or near zero) is solid.

**Also checked and ruled out as of 2026-08-22**: episode length/complexity does not separate the ~10 tasks with real success (exp0014 cumulative step 4600: `click_alarmclock`/`open_microwave`/`press_stapler`/`click_bell`/`turn_switch`/`move_playingcard_away`/`shake_bottle_horizontally`/`open_laptop`/`put_bottles_dustbin`/`shake_bottle`) from the ~40 tasks stuck at 0%. Mean episode length: 223.8 frames (successful tasks) vs 220.3 frames (failing-task sample) -- essentially identical.

**Revised implication**: the ~12% full-50-task RoboTwin ceiling (PROGRESS_0014 Section 14-15) is **not** a data-coverage limitation as previously concluded -- data volume and episode length are both roughly uniform across the successful and failing task sets. The sharp, mostly-binary per-task split (a fixed ~10-task set reaching 60-100% quickly, the rest flat at 0% even after 2000 further steps) under `expanded_projections_only`/frozen-backbone training points more toward a **representational-capacity or task-difficulty/novelty limitation** (e.g. some tasks' required bimanual motor patterns may already be representable via minor adaptation of the LIBERO-pretrained backbone's existing manifold, others may not be, given only the action_encoder/head could adapt) than a training-duration or missing-data one. This should be treated as an open, re-opened question for `$choose-fastwam-experiment`/`$investigate-fastwam-problem`, not a closed "STOP, it's a data-coverage ceiling" conclusion.

**Historical note (kept for context, methodology was unreliable, do not reuse the specific numbers)**: the original investigation (during exp0014, see PROGRESS_0014 Section 15) tried building a per-task episode-count table via naive OR-across-keywords matching and found it unreliable at fine granularity (generic-word false-positive inflation, several `place_*` variants collapsing onto the same keyword) -- correctly identified as untrustworthy for a *ranked* table, but the coarse "these 4 are at zero" conclusion drawn from the same flawed method was itself wrong, as shown above. `meta/info.json` confirms the dataset has no preserved ground-truth per-episode canonical-task label (`total_tasks: 921032` unique free-text paraphrase strings for 27,500 episodes) -- template-fragment matching against the official instruction JSONs (as used above) is a substantially more reliable substitute for the coarse zero/nonzero question than raw keyword search, though still not precise enough for a fine-grained ranked density table.
