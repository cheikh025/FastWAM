...
**Precise per-task episode-count estimation beyond this 4-task zero/nonzero check is unreliable via keyword matching -- do not trust a fine-grained per-task count table from this method.** Attempted during exp0014's investigation (why did 2000 more training steps not move the full-50-task RoboTwin mean at all -- see PROGRESS_0014 Section 14): tried building a per-task episode-count table via OR-across-keywords matching against `meta/episodes.jsonl`'s paraphrased instructions. Two concrete failure modes found: (1) forgetting to exclude a generic word from a task's keyword set causes massive false-positive inflation (e.g. `handover_block` without excluding "block" showed 3627 "hits", purely from unrelated tasks like `stack_blocks`/`beat_block_hammer` -- contradicts the reliable 0-hit finding above); (2) several `place_*` task variants (`place_a2b_left`, `place_a2b_right`, `place_fan`, ...) reduce to the same generic keyword ("place") once their one distinguishing object noun is stripped by exclusion, so their counts become indistinguishable from each other and are dominated by the ultra-common verb "place" rather than the actual object. **The paraphrased-instruction dataset has no preserved ground-truth per-episode canonical-task label** (`meta/info.json` shows `total_tasks: 921032` unique paraphrase strings for only 27,500 episodes -- confirms these are free-text LLM paraphrases, not structured task IDs), so precise per-task density cannot be reliably reconstructed this way. Only the coarse "does this task have ~0 hits under a strict single/paired-keyword substring check" style claim (as used for the 4-task finding above) should be trusted from this dataset; a fine-grained ranked table should not be.
