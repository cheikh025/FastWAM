...
- verified local checkpoint path: was `/workspace/FastWAM/checkpoints/exp0019_parent_hf/promoted/0019_spatial_weak_task_oversampling/step_005000.pt` (also fetched alongside: `dataset_stats.json`, `libero_2cam_plus90_reweighted_weaktasks_v3.yaml`, `libero_uncond_2cam224_plus90_reweighted_weaktasks_v3_3e-5.yaml`, `PROGRESS_0019_spatial_weak_task_oversampling.md`) — **deleted during exp0001's disk crisis** (see "Disk crisis" in `research/NOTES.md`) to free space; durably backed up on `cheikh025/ASR`, verified downloadable via the exact command in "Durable checkpoint storage" below. The checkpoint-expansion output (`checkpoints/exp0019_expanded_k14/step_005000.pt`) is what actually matters for continued training and was NOT deleted.
