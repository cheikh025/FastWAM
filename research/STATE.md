...
Status: `RESEARCH_LOOP_ACTIVE — exp0003 rejected (surprising: fix didn't beat exp0001), investigating next hypothesis`
...
`0003_disjoint_action_offset` — `REJECT`. Full report: `research/progress/PROGRESS_0003_disjoint_action_offset.md`. Implemented disjoint per-embodiment column offsets (K 14->21 action / 14->22 proprio; LIBERO at offset 0, RoboTwin at offset 7/8) to eliminate the shared-weight-column overlap diagnosed after exp0001/exp0002 — proven interference-free in isolation by a gradient-isolation unit test, and the training run itself was clean (1000/1000 steps, no NaN/anomalies). **Surprising result**: LIBERO-Spatial screen landed at 50.00% (15/30) — better than exp0002 (16.67%) but *worse* than exp0001 (73.33%), contradicting the simple version of the hypothesis (disjoint projections + same trainable backbone as exp0001 should have been >= exp0001's retention if projection overlap were the sole mechanism). Per-task pattern is scattered/task-swapping rather than a uniform decline, consistent with either meaningful run-to-run variance on the small 3-trial/task panel or a new instability source (RoboTwin's all-fresh-init projection weights vs. exp0001's partial warm-start). Combined with exp0002, points toward the shared transformer backbone as an under-isolated interference channel — no candidate has yet combined a frozen backbone with disjoint-offset projections to test this cleanly. Next: `$investigate-fastwam-problem` before choosing exp0004.
