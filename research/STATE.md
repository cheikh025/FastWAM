...
Status: `RESEARCH_LOOP_ACTIVE — DECISIVE FINDING: retention problem is NOT primarily multi-embodiment interference (exp0009); pivoting to optimizer/LR stability (exp0010)`
...
## Latest experiment — DECISIVE FINDING

`0009_libero_only_control` — `REJECT` (diagnostic control). Full report: `research/progress/PROGRESS_0009_libero_only_control.md`. Resumed from the same K=21/22 expanded checkpoint every recent candidate used, but trained on **LIBERO data alone — zero RoboTwin exposure at all**. Result: LIBERO-Spatial **23.33% (7/30) — the second-worst result of any candidate in the project**, worse than 6 of 8 multi-embodiment candidates. **This falsifies the multi-embodiment-interference framing (projection overlap, backbone drift, mixing ratio) as the primary cause of the retention problem that drove every candidate since exp0002.** The real driver is a property of continuing to fine-tune this expanded/padded K=21/22 checkpoint, independent of RoboTwin's presence. Leading hypothesis: cold-start Adam optimizer instability (weights-only resume restarts Adam's moment estimates at zero; bias-correction amplifies effective step size for roughly the first 10-50 steps against an already-converged solution) and/or the LR (`3e-5`) simply being too high for continued fine-tuning. Next: exp0010, same LIBERO-only setup at a 10x lower LR (`3e-6`) to directly test this — if retention holds up, the fix for the whole project is "use a much lower LR," and prior architecture/mixing candidates should be revisited at the corrected LR.
...
- `0009_libero_only_control` — `REJECT` (diagnostic control, decisive finding). Resumed the K=21/22 expanded checkpoint and trained on LIBERO alone (zero RoboTwin exposure). Result: LIBERO-Spatial **23.33%**, the second-worst of any candidate — **falsifies multi-embodiment interference as the primary retention-loss mechanism**; the degradation is a property of continued fine-tuning on this architecture, independent of RoboTwin. Leading hypothesis: cold-start Adam optimizer instability and/or too-high LR for continued fine-tuning of an already-converged checkpoint. Full report: `research/progress/PROGRESS_0009_libero_only_control.md`.
