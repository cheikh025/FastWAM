...
`0005_disjoint_offset_backbone_low_lr` — `REJECT`. Full report: `research/progress/PROGRESS_0005_disjoint_offset_backbone_low_lr.md`. Tested whether partial backbone plasticity (10x lower LR than the projection layers, not fully frozen) could preserve exp0004's retention while unlocking more RoboTwin learning capacity — a new `Trainer.trainable_modules="dit_with_backbone_low_lr"` mode with a two-parameter-group AdamW optimizer. Result: LIBERO-Spatial 63.33% (matched exp0004's aggregate exactly, different per-task mix), but the RoboTwin progress check went the **wrong direction**: `adjust_bottle` stayed 0%/0% (unchanged), and `click_alarmclock` **lost** its randomized-phase capability entirely (33.3%->0.0%, exp0004's one real success), keeping only the clean-phase result. Partial plasticity cost RoboTwin robustness for no gain — the opposite of the hypothesis. Two open questions before exp0006: is exp0004's 33.3%/33.3% itself just n=3-episode noise, and if real, is training budget (not the LR-ratio dial) the more relevant lever, since RoboTwin's own components got the same ~500 gradient steps regardless of backbone LR. Next: `$investigate-fastwam-problem`.
