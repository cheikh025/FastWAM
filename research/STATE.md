...
**Decision**: `STOP_TRAINING` this trajectory (exp0014). `DIAGNOSE` next -- investigate actual per-task episode counts/density in the RoboTwin training data (`data/robotwin2.0/robotwin2.0/meta/episodes.jsonl`, only spot-checked for the 4 fully-absent tasks so far) before designing the next candidate. Working hypothesis: most of the ~40 weak/zero tasks have thin/imbalanced per-task representation relative to the ~10 tasks with real capability, and this needs to be addressed directly (per-task oversampling/reweighting, or acquiring more source data for underrepresented tasks) rather than solved by more steps on the same mixture.
