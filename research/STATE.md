...
`0006_disjoint_offset_frozen_backbone_4k` — `REJECT`. Full report: `research/progress/PROGRESS_0006_disjoint_offset_frozen_backbone_4k.md`. Re-ran exp0004's exact recipe (frozen backbone, disjoint-offset K=21/22 projections, no new variable) for 4000 steps instead of 1000 — directly testing whether training budget (not the LR-ratio dial exp0005 tried) was the missing lever. Result: LIBERO-Spatial 63.33%, exactly matching exp0004 and exp0005 (three candidates now converge on this exact 19/30 ceiling regardless of training treatment). RoboTwin: `click_alarmclock` **lost its only real capability entirely** (33.3%/33.3% at step 1000 -> 0.0%/0.0% at step 4000, confirmed via raw result files); `adjust_bottle` stayed dead. Both exp0005 (different LR) and exp0006 (more steps, same recipe) actively destroyed exp0004's one real result rather than building on it — strong evidence exp0004's checkpoint sits near a narrow, fragile optimum, not a stable/improvable capability. Also documented a real infra gotcha: a full LIBERO screen run concurrently with an active `KEEP=1` checkpoint pruner can race and crash (see `research/NOTES.md`). Next: `$investigate-fastwam-problem` — specifically, does exp0004's `click_alarmclock` result even reliably replicate, before assuming it was a real capability rather than a lucky single measurement.
