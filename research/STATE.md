...
**Full 50-task RoboTwin Clean-only scan complete (n=10, ~8h51m wall-clock, 2026-08-19 08:49-17:41)**: **12.6% mean success across all 50 tasks** -- far below the curated 4-task panel's 75% and far below the 90% target. Only 6 of 50 tasks clear 50%: `click_bell`/`open_microwave`/`press_stapler` (100%), `turn_switch` (90%), `click_alarmclock` (80%), `shake_bottle_horizontally` (50%). The other 44 are weak-to-zero, including 4 tasks with zero training-data coverage. Full breakdown: `research/progress/PROGRESS_0014_frozen_backbone_diagnostic.md` Section 10.

**This resolves the promotion-tradeoff question raised earlier**: the gap to the project's 90% full-50-task target is not primarily a close tradeoff call (a suite dipping under 90% vs. gains elsewhere) -- it's a training-scale/coverage gap. The model has learned real capability on a handful of well-represented tasks, but has not yet had enough training exposure across the full 50-task benchmark. Combined with the LIBERO-Long regression (87.0% vs 95.0% baseline), the honest read is: this checkpoint is an early, promising proof of the frozen-backbone recipe, not a promotion candidate. Next steps should focus on continued/scaled training (more steps, revisit data coverage/weighting across all 50 tasks) rather than a promotion decision now.
