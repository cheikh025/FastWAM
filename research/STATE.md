...
`0007_exp0004_replication` — `REJECT`. Full report: `research/progress/PROGRESS_0007_exp0004_replication.md`. Re-ran exp0004's exact recipe fresh (no code/config changes) and evaluated RoboTwin's `click_alarmclock` with 10 episodes/phase instead of the usual 3, to settle whether exp0004's original 33.3%/33.3% capability was real or a lucky n=3 measurement. **Result: 20.0% clean / 10.0% random, confirmed via raw result files — the capability is real.** A true-0% model cannot produce real, physically-verified task completions across independent seeds; an independently-trained checkpoint from the same recipe also shows clear non-zero success on both phases, just at a noisier, somewhat lower rate than the original single n=3 draw. `adjust_bottle` remains a clean 0% (now 4 candidates, 12 episodes, zero successes). LIBERO-Spatial: 56.67%, close to but not exactly the 63.33% seen in exp0004/0005/0006 (confirms real run-to-run variance despite a near-identical training loss). This reframes exp0005/exp0006's apparent 0% "regressions" as ambiguous (could be genuine destruction, or unlucky n=3 draws from a genuinely modest, noisy true rate) — the dominant unresolved problem remains LIBERO retention, stuck in a 56-63% band across four independent runs of the disjoint-offset-frozen-backbone recipe family, with exp0001 (73.33%, trainable backbone + *overlapping* projections) still the single best retention result of all 7 candidates. Also hit and documented a second real infra gotcha: `run_robotwin_manager.py` ignores `CUDA_VISIBLE_DEVICES` remapping to non-zero GPU indices, causing an OOM when attempting to parallelize with a concurrent LIBERO screen (`research/NOTES.md`). Next: `$choose-fastwam-experiment` — combine the interference-free disjoint-offset projections with a LIBERO-favoring data-mixing ratio (untested variable) on a trainable backbone.
