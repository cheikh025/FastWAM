...
Status: `RESEARCH_LOOP_ACTIVE — exp0004 rejected (best frozen-backbone retention yet, 63.33%), RoboTwin progress check running, designing exp0005`
...
`0004_disjoint_offset_frozen_backbone` — `REJECT`. Full report: `research/progress/PROGRESS_0004_disjoint_offset_frozen_backbone.md`. Combined exp0002's backbone freezing with exp0003's disjoint-offset projections — the missing cell in the 2x2 matrix. Result: LIBERO-Spatial **63.33%** (19/30), a huge jump over exp0002's 16.67% and above exp0003's 50.00%, but still below exp0001's 73.33% and the 90% floor. The completed matrix reveals freezing has *opposite* effects depending on projection design: hurts with overlapping projections (73.33%->16.67%), helps with disjoint projections (50.00%->63.33%) — confirms disjoint offsets are a real fix and backbone drift is a second, independent interference channel. A RoboTwin progress check (same 2-task panel as exp0001) is running on exp0004's checkpoint to see whether a frozen backbone can support any RoboTwin learning at all. Next: design exp0005 informed by both results.
...
- `0004_disjoint_offset_frozen_backbone` — `REJECT`. Combined exp0002's backbone freezing (`trainable_modules=expanded_projections_only`) with exp0003's disjoint-offset projections — fills the missing 2x2 matrix cell. Result: LIBERO-Spatial **63.33%** (19/30) — far above exp0002 (16.67%), above exp0003 (50.00%), still below exp0001 (73.33%) and the 90% floor. **Completed matrix**: overlap+trainable=73.33%, overlap+frozen=16.67%, disjoint+trainable=50.00%, disjoint+frozen=63.33%. Freezing has opposite effects by projection design (hurts with overlap, helps with disjoint) — confirms disjoint offsets are a real, substantial fix and backbone drift is a second, independent interference channel that a full freeze eliminates at the cost of whatever adaptation benefit backbone plasticity provides (exp0001 remains the best retention result despite imperfect projection design, suggesting some net-positive value to backbone plasticity not yet isolated). Full report: `research/progress/PROGRESS_0004_disjoint_offset_frozen_backbone.md`.
