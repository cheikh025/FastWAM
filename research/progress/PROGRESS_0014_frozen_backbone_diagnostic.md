...
**Attempted `DIAGNOSE` on per-task RoboTwin data density, hit a real methodology wall**: tried to build a per-task episode-count table via keyword-matching `meta/episodes.jsonl`'s paraphrased instructions against the 50 canonical task names. Found this approach unreliable beyond the coarse "4 tasks have ~0 hits" claim already established -- two concrete failure modes (generic-word false-positive inflation, and several `place_*` variants collapsing onto the same generic keyword once their distinguishing noun is excluded) mean a fine-grained ranked table cannot be trusted. `meta/info.json` confirms the dataset has no preserved ground-truth per-episode canonical-task label (`total_tasks: 921032` unique free-text paraphrases for 27,500 episodes) -- full writeup in `research/NOTES.md`. **Do not reuse a naive keyword-count table from this dataset as evidence in future candidates.**

Given precise data-coverage diagnosis isn't reliably recoverable this way, the more productive next step is an **actionable experiment** rather than further data archaeology: the frozen-backbone recipe leaves LIBERO retention essentially risk-free (100% throughout, since the backbone cannot drift regardless of what RoboTwin data does), which was not true under exp0013's full-backbone regime (where a higher RoboTwin ratio was previously found risky for LIBERO, per exp0008's 3:1-ratio result on the old exp0019 lineage). This removes the original reason for keeping a conservative 1:1 LIBERO:RoboTwin mixing ratio. **Recommended next candidate**: resume from exp0014's cumulative-4600 checkpoint with a RoboTwin-heavier (or RoboTwin-only) sampling ratio under the same frozen-backbone setting, to test whether more effective RoboTwin gradient exposure per step broadens capability beyond the current ~10-task ceiling, without the LIBERO risk that made this untested under the old full-backbone regime.
