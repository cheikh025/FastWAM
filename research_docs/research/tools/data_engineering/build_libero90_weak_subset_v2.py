...
DST = Path("/workspace/fastwam_libero90/FastWAM/data/libero_mujoco3.3.2/libero_90_weak_subset_v2_lerobot")
...
TARGET_TASKS = [
    ...
    # newly-emerged weak tasks in exp0017's canonical eval
...
def main():
    ...
    n = len(selected)
    assert n <= chunks_size, f"subset ({n}) exceeds chunks_size ({chunks_size}); would need multi-chunk logic"

    # fresh task list + index
    ...
    for new_idx, e in enumerate(selected):
        ...
        # rewrite parquet: episode_index/task_index/index columns must match the
        # new renumbering (they are baked into the row data, not just the file path/meta)
        ...
        # symlink videos (no internal episode metadata to fix -- pure frame data)
        for vk in video_keys:
