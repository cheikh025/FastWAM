...
SRC = Path("/workspace/fastwam_libero90/FastWAM/data/libero_mujoco3.3.2/libero_90_no_noops_lerobot")

# stove+pan (task21 eval) + wine rack (task27 eval) -- both never before oversampled
DST_V3 = Path("/workspace/fastwam_libero90/FastWAM/data/libero_mujoco3.3.2/libero_90_weak_subset_v3_lerobot")
TARGET_TASKS_V3 = [
    "turn on the stove and put the frying pan on it",
    "put the wine bottle on the wine rack",
]

# book-right-compartment (task75 eval) -- isolated for intensified oversampling
DST_BOOK = Path("/workspace/fastwam_libero90/FastWAM/data/libero_mujoco3.3.2/libero_90_book_right_intensify_lerobot")
TARGET_TASKS_BOOK = [
...
def build(dst, target_tasks, episodes, ep_stats, info, chunks_size, video_keys):
    ...
    print(f"[{dst.name}] selected {len(selected)} episodes out of {len(episodes)}")
    by_task = {}
    for e in selected:
    ...
    n = len(selected)
    assert n <= chunks_size, f"subset ({n}) exceeds chunks_size ({chunks_size})"
    assert n > 0, f"no episodes matched for {dst.name}"
    ...
    print(f"[{dst.name}] wrote filtered dataset: total_episodes={n} total_frames={total_frames} total_tasks={len(unique_tasks)}\n")
...
def main():
    ...
    build(DST_V3, TARGET_TASKS_V3, episodes, ep_stats, info, chunks_size, video_keys)
    build(DST_BOOK, TARGET_TASKS_BOOK, episodes, ep_stats, info, chunks_size, video_keys)
